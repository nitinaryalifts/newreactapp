    ...
    const fetchMetaTags = async () => {
      try {
        const response = await fetch(
          "https://mancuso.ai/mancusov2/wp-json/wp/v2/pages"
        );
        if (!response.ok) {
        ...
        if (data && data.length > 0) {
          ...
          const limitedDescription =
            getLimitedDescription(personOrgDescription);
  ...
  return (
    <>
      <div className={`main_Content `}>
        <div className={`portfolio animate_section ${animationClass}`}>
          <section
            className={`portfolio_section section_padding py-5 bg-white`}
          >
